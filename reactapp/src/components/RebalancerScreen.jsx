...
const AnimatedNumber = ({ value, duration = 800 }) => {
  ...
  useEffect(() => {
    ...
    const finalStepTime = Math.max(stepTime, 16);
    const stepCount = duration / finalStepTime;
    const valIncrement = range / stepCount;

    const timer = setInterval(() => {
      current += valIncrement;
      if ((valIncrement > 0 && current >= end) || (valIncrement < 0 && current <= end)) {
        setDisplayValue(end);
        clearInterval(timer);
      } else {
